...
def basic_clean(
    ...
    # Remove unusual words if not token with alphabetic characters only
    if remove_unusual:
...
def generate_list_of_grams(documents_df, text_col):
    ...
    bigram = Phrases(
        texts, min_count=10, threshold=5, connector_words=ENGLISH_CONNECTOR_WORDS
    )
