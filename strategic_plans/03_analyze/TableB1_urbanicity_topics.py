...
wb = Workbook()
ws = wb.active
ws.title = "Urbanicity Topic Analysis"

# Set up headers
headers = ["Topic", "Urban", "Suburb", "Town", "Rural", "Adj P-value"]
...
p_values = []
topic_results = []
urbanicity_cols = ["urban", "suburb", "town", "rural"]
...
for topic_code in ordered_topics:
    ...
    # Dictionary to store raw values by urbanicity
    urbanicity_data_dict = {}

    for urbanicity in urbanicity_cols:
        group_df = merge_df[merge_df[urbanicity] == 1]
        values = group_df[topic_id].values if len(group_df) > 0 else np.array([])
        urbanicity_data_dict[urbanicity] = values

    # Calculate means and standard deviations
    means = {}
    stds = {}
    for urbanicity in urbanicity_cols:
        vals = urbanicity_data_dict[urbanicity]
        mean_val = np.nan if len(vals) == 0 else np.nanmean(vals)
        std_val = np.nan if len(vals) == 0 else np.nanstd(vals, ddof=1)
        means[urbanicity] = mean_val
        stds[urbanicity] = std_val

    # Perform ANOVA
    valid_groups = [
        pd.Series(vals).dropna().values
        for vals in urbanicity_data_dict.values()
    ...
    topic_long_df = pd.DataFrame([
        {"urbanicity": u, "value": v, "topic": topic_code}
        for u, vals in urbanicity_data_dict.items()
        for v in vals
    ])
...
for i, result in enumerate(topic_results):
    ...
    # Add means to Excel
    for col_idx, urbanicity in enumerate(urbanicity_cols, 2):
        mean_val = result['means'][urbanicity]
    ...
    for col_idx, urbanicity in enumerate(urbanicity_cols, 2):
        std_val = result['stds'][urbanicity]
        ws.cell(row=row, column=col_idx,
...
for col_idx, urbanicity in enumerate(urbanicity_cols, 2):
    count = len(merge_df[merge_df[urbanicity] == 1])
...
for urbanicity in urbanicity_cols:
    count = len(merge_df[merge_df[urbanicity] == 1])
    print(f"  {urbanicity.capitalize()}: {count} districts")