...
# ---------------- Score quartiles ----------------
merge_df['score_quartile'] = pd.qcut(
    merge_df['mean_test_score'], q=4, labels=['Q1', 'Q2', 'Q3', 'Q4']
)
...
for q in ['Q1', 'Q2', 'Q3', 'Q4']:
    ...
    print(f"{q}: {s['min']:.2f} - {s['max']:.2f} (mean: {s['mean']:.2f}, n={int(s['count'])})")
...
for topic_code in ordered_topics:
    ...
    formula = (
        f"{topic_code} ~ C(score_quartile, Treatment(reference='Q1')) "
        f"+ C(state) + improvement_plan + form_plan + word_count"
    )
    model = smf.ols(formula, data=merge_df, missing='drop').fit()

    q1_mean = merge_df.loc[merge_df['score_quartile'] == 'Q1', topic_code].mean()

    coefficients = {
        lvl: model.params.get(f"C(score_quartile, Treatment(reference='Q1'))[T.{lvl}]", np.nan)
        for lvl in quartile_levels
    }
    std_errors = {
        lvl: model.bse.get(f"C(score_quartile, Treatment(reference='Q1'))[T.{lvl}]", np.nan)
        for lvl in quartile_levels
    }

    # Explicit joint F-test
    f_test = model.f_test(
        "C(score_quartile, Treatment(reference='Q1'))[T.Q2] = 0, "
        "C(score_quartile, Treatment(reference='Q1'))[T.Q3] = 0, "
        "C(score_quartile, Treatment(reference='Q1'))[T.Q4] = 0"
    )
...
# ---------------- Notes & Save ----------------
ws.cell(row=current_row + 1, column=1, value="Note: Q1 (lowest test scores) is the reference category").font = Font(italic=True)
if NORMALIZED:
    ws.cell(row=current_row + 2, column=1, value="Prevalence values normalized by sum of included topics").font = Font(italic=True)

output_path = start.RESULTS_DIR + 'Table9_score_quartiles_coefficients.xlsx'
wb.save(output_path)
