...
wb = Workbook()
ws = wb.active
ws.title = "Politics Quartiles Topic Analysis"
...
for topic_code in ordered_topics:
    ...
    for quartile in quartile_labels:
        group_df = merge_df[merge_df['politics_quartile'] == quartile]
...
for col_idx, quartile in enumerate(quartile_labels, 2):
    count = len(merge_df[merge_df['politics_quartile'] == quartile])
    ws.cell(row=current_row + 1, column=col_idx, value=str(count))

# %%
# Add characteristic means row
ws.cell(row=current_row + 2, column=1, value="Mean % Trump Vote")
ws.cell(row=current_row + 2, column=1).font = Font(bold=False)

for col_idx, quartile in enumerate(quartile_labels, 2):
    quartile_data = merge_df[merge_df['politics_quartile'] == quartile]
    if len(quartile_data) > 0:
        mean_characteristic = quartile_data['district_pct_trump'].mean()
...
# %%
# Save Excel results
output_path = start.RESULTS_DIR + 'AppendixB6_politics_quartiles.xlsx'
wb.save(output_path)
