...
# ---------------- Income quartiles ----------------
# Convert back to full $ (variable was medinc_1000)
merge_df['medinc_1000'] = merge_df['medinc_1000'] / 1000
merge_df['income_quartile'] = pd.qcut(
    merge_df['medinc_1000'], q=4, labels=['Q1', 'Q2', 'Q3', 'Q4']
)
...
for q in ['Q1', 'Q2', 'Q3', 'Q4']:
    ...
    print(f"{q}: ${s['min']*1000:,.0f} - ${s['max']*1000:,.0f} "
          f"(mean: ${s['mean']*1000:,.0f}, n={int(s['count'])})")
...
wb = Workbook()
ws = wb.active
ws.title = "Income Coefficients (State FE)"
...
for topic_code in ordered_topics:
    ...
    formula = (
        f"{topic_code} ~ C(income_quartile, Treatment(reference='Q1')) "
        f"+ C(state) + improvement_plan + form_plan + word_count"
    )
    model = smf.ols(formula, data=merge_df, missing='drop').fit()

    q1_mean = merge_df.loc[merge_df['income_quartile'] == 'Q1', topic_code].mean()

    coefficients = {
        lvl: model.params.get(f"C(income_quartile, Treatment(reference='Q1'))[T.{lvl}]", np.nan)
        for lvl in quartile_levels
    }
    std_errors = {
        lvl: model.bse.get(f"C(income_quartile, Treatment(reference='Q1'))[T.{lvl}]", np.nan)
        for lvl in quartile_levels
    }

    # Explicit joint F-test
    f_test = model.f_test(
        "C(income_quartile, Treatment(reference='Q1'))[T.Q2] = 0, "
        "C(income_quartile, Treatment(reference='Q1'))[T.Q3] = 0, "
        "C(income_quartile, Treatment(reference='Q1'))[T.Q4] = 0"
    )
...
# ---------------- Notes & Save ----------------
ws.cell(row=current_row + 1, column=1, value="Note: Q1 (lowest income) is the reference category").font = Font(italic=True)
if NORMALIZED:
...
print("Reference category: Q1 (lowest income)")
